...
public partial class TaskDetailPageModel : ObservableObject, IQueryAttributable
{
    ...
    private async Task LoadTaskAsync(IDictionary<string, object> query)
    {
        ...
        {
            taskId = Convert.ToInt32(value, CultureInfo.InvariantCulture);
            _task = await _taskRepository.GetAsync(taskId).ConfigureAwait(false);

            if (_task is null)
            {
                _errorHandler.HandleError(new ArgumentException($"Task Id {taskId} isn't valid."));
                return;
            }

            Project = await _projectRepository.GetAsync(_task.ProjectID).ConfigureAwait(false);
        }
        else
        {
        ...
        if (taskId > 0)
        {
            if (_task is null)
            {
                _errorHandler.HandleError(
                    new ArgumentException($"Task with id {taskId} could not be found.")
                );
                return;
            }
    ...
    private async Task Save()
    {
        if (_task is null)
        {
            _errorHandler.HandleError(
                new InvalidOperationException(
                    "Task or project is null. The task could not be saved."
                )
            );
        ...
        await Shell.Current.GoToAsync("..?refresh=true").ConfigureAwait(false);

        if (_task.ID > 0)
        {
            await AppShell.DisplayToastAsync("Task saved").ConfigureAwait(false);
        }
    }
    ...
    private async Task Delete()
    {
        ...
        {
            _errorHandler.HandleError(
                new InvalidOperationException("Task is null. The task could not be deleted.")
            );
        ...
        if (_task.ID > 0)
        {
            await _taskRepository.DeleteItemAsync(_task).ConfigureAwait(false);
        }

        await Shell.Current.GoToAsync("..?refresh=true").ConfigureAwait(false);
        await AppShell.DisplayToastAsync("Task deleted").ConfigureAwait(false);
    }
}
